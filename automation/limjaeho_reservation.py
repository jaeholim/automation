    ...
    for sn in seat_nominees:
        try:
            ...
            try:
                ...
                seat = driver.find_element_by_xpath("//*[contains(@title,'{}') and @class='stySelectSeat']".format(sn))
                seat.click()
                continue
...
driver.get("http://ticket.interpark.com/search/ticket.asp?search=%uC548%uC0B0%uD654%uB791%uC624%uD1A0%uCEA0%uD551%uC7A5")

info_dates = driver.find_elements_by_xpath("//*[@id='play_list']/*/td[@class='info_Date']")
idx = 1;
for info_date in info_dates:
    print("Row Num : {}, available date : {}".format(idx, info_date.text))
    sdate, edate = info_date.text.split("~")
    if datetime.strptime(sdate.strip(), "%Y.%m.%d") <= reserve_date and reserve_date <= datetime.strptime(edate.strip(), "%Y.%m.%d"):
        driver.find_element_by_xpath("//*[@id='play_list']/tr[{}]/td[@class='btnArea']/a[1]".format(idx)).click()
        break
    idx+=1
...
# 예약 페이로 이동
for rd, rdur in reserve_days.items():
    ...
    while not is_possible:
        ...
        while not is_correct_month:
            ...
            if current_month == reserve_month:
...
#                 payment_complete()
                ...
                print("{} 자리가 하나도 없거나 이미 예를 했음".format(seat_nominees))
