...
#                      time.sleep(0.2)
    ...
    def process_reservation(self):
        ...
        # 테스트용 코드 - 원하는 시간이 될때까지 기다림
        wait_datetime = datetime.strptime("2019-06-07 13:00:00", "%Y-%m-%d %H:%M:%S")
        ...
        loop_idx = 1
        while self.loop_count <= 0 or ( self.loop_count > 0 and loop_idx <= self.loop_count):
            print("=======================================================================")
            print("===== {}/{} 번째 시도 ".format(loop_idx, "∞" if self.loop_count <= 0 else self.loop_count))
            print("=======================================================================")
            loop_idx+=1
            ...
            e_time = time.time()
            print("--------- search duration : {}".format(e_time - s_time))
            s_time = time.time()
    ...
    #             print("Row Num : {}, available date : {}".format(idx, info_date.text))
                sdate, edate = [datetime.strptime(d.strip(), "%Y.%m.%d") for d in info_date.text.split("~")]
                if sdate <= self.reserve_date and edate:
    #                 print("before temp_date : {}, select_idx : {}, sdate : {}, edate : {}".format(temp_date, select_idx, sdate, edate))
                    if temp_date is None or temp_date < sdate:
                        select_idx = idx
                        temp_date = sdate
        #                 self.driver.find_element_by_xpath("//*[@id='play_list']/tr[{}]/td[@class='btnArea']/a[1]".format(idx)).click()
        #                 break
    #                 print("after temp_date : {}, select_idx : {}, sdate : {}, edate : {}".format(temp_date, select_idx, sdate, edate))
                idx+=1
                
            if select_idx > 0:
                self.driver.find_element_by_xpath("//*[@id='play_list']/tr[{}]/td[@class='btnArea']/a[1]".format(select_idx)).click()
...
#                     time.sleep(0.5)
                    ...
                    if len(possible_links) > 0:
                        ...
                        if self.date_seat_select(rd, rdur):
                            ...
                            print("{} 자리가 하나도 없거나 이미 예약을 했음".format(self.seat_nominees))
                        ...
                        self.driver.refresh()
            e_time = time.time()
            print("--------- reservation duration : {}".format(e_time - s_time))
